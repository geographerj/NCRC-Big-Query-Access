-- Comerica Bank Small Business Lending Analysis
...
subject_volume AS (
    ...
    WHERE l.sb_rssd = '60143'  -- Comerica Bank
...
top_10_cbsas AS (
    -- Find top 10 CBSAs by Comerica SB loan volume across all years
    SELECT 
        d.msamd as cbsa_code
    FROM `hdma1-242116.sb.disclosure` d
    INNER JOIN `hdma1-242116.sb.lenders` l
...
peers AS (
    ...
    WHERE al.sb_rssd != '60143'  -- Exclude Comerica
...
subject_data AS (
    ...
    WHERE sb_rssd = '60143'  -- Comerica Bank
