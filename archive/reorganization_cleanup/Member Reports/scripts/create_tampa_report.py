"""
Create complete Tampa market report (PDF + Excel)
...
def load_tampa_data():
    """Load processed Tampa data"""
    data_dir = member_reports_dir / "data" / "processed"
    
    market_metrics = pd.read_csv(data_dir / "tampa_market_metrics.csv")
    top_10_metrics = pd.read_csv(data_dir / "tampa_top_10_lenders_metrics.csv")
    top_lenders = pd.read_csv(member_reports_dir / "data" / "raw" / "tampa_top_lenders_2024.csv")
    
    return market_metrics, top_10_metrics, top_lenders
...
def format_lender_name(name: str) -> str:
    """Format lender name by converting to all uppercase"""
...
def create_top_lenders_tables(top_10_metrics: pd.DataFrame, top_lenders: pd.DataFrame, lender_info: pd.DataFrame = None) -> dict:
    ...
    for lei in top_10_leis:
        ...
        # Get lender name and format it
        lender_name_raw = lender_data.iloc[0].get('respondent_name', lei)
        if pd.isna(lender_name_raw) or lender_name_raw == '':
            lender_name = lei
        else:
...
def generate_key_findings(market_metrics: pd.DataFrame) -> list:
    ...
    # Overall volume
    findings.append(
        f"Lenders originated {int(total_originations):,} home purchase loans in Tampa city from {earliest_year} through {latest_year}."
    )
...
def load_community_profile_data() -> dict:
    """Load community profile data from processed JSON file"""
    profile_path = member_reports_dir / "data" / "processed" / "tampa_community_profile_data.json"
...
def generate_introduction(community_data: dict = None) -> str:
    ...
    intro = "This report examines mortgage lending patterns in Tampa, Florida from 2018 through 2024, focusing on home purchase originations within Tampa city boundaries. The analysis encompasses 130 census tracts and evaluates lending outcomes by borrower demographics and geographic characteristics, using standard fair lending metrics.\n\n"
    ...
    if community_data:
        ...
        if demographics.get('total_population'):
            ...
            if pop > 1000:  # Sanity check - avoid small numbers that might be errors
                demo_parts.append(f"According to the Community Profile of Tampa, FL (City, 2020), Tampa city has a population of approximately {pop:,}")
        ...
        if demo_percentages:
            if demo_parts:
                ...
                intro += f"According to the Community Profile of Tampa, FL (City, 2020), the city's population is {', '.join(demo_percentages)}. "
        ...
        if trends.get('demographic_shifts'):
            ...
            intro += "The community profile documents significant growth in the Hispanic and Latino population over recent years, reflecting broader demographic shifts across Florida. "
    
    if not community_data or not community_data.get('demographics'):
        # Fallback if no community profile data
        intro += "Tampa is a diverse metropolitan area with a growing population that reflects broader demographic trends in Florida. The city has experienced significant growth in Hispanic and Latino residents over the past decade, while maintaining a substantial White population and smaller but important Black, Asian, and other racial and ethnic communities. "
    
    intro += "Demographic shifts in Tampa mirror state and national patterns, with increasing diversity contributing to changes in housing demand and mortgage lending patterns.\n\n"
...
def generate_market_patterns_narrative(market_metrics: pd.DataFrame, community_data: dict = None) -> dict:
    ...
    if community_data and community_data.get('demographics'):
        ...
        if demo_parts:
            lead_in = (
                f"According to the Community Profile of Tampa, FL (City, 2020), Tampa city's population is composed of "
    ...
    # Build analysis in plain English with community profile context
    analysis_parts = []
    ...
    if abs(hispanic_change) > 5:
        analysis_parts.append(
            f"Lending to Hispanic borrowers expanded significantly over the period. This growth reflects Tampa's "
        ...
        if comm_hispanic:
            ...
            if abs(gap) > 10:  # Only cite specific gap if very large
                if gap < 0:
                    analysis_parts.append(
                        ...
                        f"in the community's population. According to the Community Profile of Tampa, FL (City, 2020), "
                        f"Hispanic and Latino residents make up a larger portion of Tampa's population than their share "
                        f"of mortgage lending, indicating continued room for improvement in credit access."
                    )
                else:
                    analysis_parts.append(
                        f"This lending share now exceeds the Hispanic population share in Tampa, reflecting strong "
                ...
                analysis_parts.append(
                    f"This lending pattern now aligns with or exceeds the Hispanic population share in Tampa, "
            ...
            analysis_parts.append(
                f"This expansion likely reflects both demographic changes in Tampa's population and improved "
    ...
    if abs(black_change) < 2:
        ...
        if comm_black:
            ...
            if abs(black_gap) > 10:  # Only cite specific gap if very large
                analysis_parts.append(
                    f"The share of loans going to Black borrowers falls well below their representation in Tampa's population. "
                    f"The Community Profile of Tampa, FL (City, 2020) shows that Black residents make up a substantially "
                    f"larger portion of the city's population than their share of mortgage lending would suggest. "
                ...
                analysis_parts.append(
                    ...
                    f"their representation in Tampa's population."
                )
        else:
            analysis_parts.append(
    ...
    if lmib_change < -2:
        ...
        if poverty_rate:
            geo_analysis.append(
                f"Given that Tampa has a significant portion of residents living below the poverty line, "
    ...
    if abs(lmict_change) > 5:
        ...
        if lmib_change < -2 and lmict_change > 5:
            ...
            if homeownership_rate:
                ...
                geo_analysis.append(
                    f"According to the Community Profile of Tampa, FL (City, 2020), approximately {renter_rate:.0f}% of "
                    f"Tampa households are renters. This means that many residents in lower-income neighborhoods "
    ...
    if abs(mmct_change) > 5:
        ...
        if comm_black or comm_hispanic:
            geo_analysis.append(
                f"Given Tampa's diverse population, this increase in lending to majority-minority areas reflects "
    ...
    analysis_parts.append(
        ...
        f"opportunities to expand equitable access to mortgage credit throughout Tampa's diverse communities."
    )
...
representing {market_share:.1f}% of all home purchase originations in Tampa city that year. 
...
def generate_lender_lead_in(lender_info_row: pd.Series, lender_name: str, background_info: dict = None) -> str:
    ...
    if background_info and background_info.get('headquarters'):
        ...
        lead_in = f"{lender_name} is one of the top lenders in the Tampa market."
    else:
...
def main():
    ...
    market_metrics, top_10_metrics, top_lenders = load_tampa_data()
        ...
        if background_data_file.exists():
            try:
                with open(background_data_file, 'r') as f:
                    ...
                    if lender_key:
                        # Use stored data directly (already extracted)
                        stored_data = all_background_data[lender_key]
                        background_info = {
                            'headquarters': stored_data.get('headquarters'),
                            'history': stored_data.get('history'),
                            'mergers': stored_data.get('mergers'),
                            'website': stored_data.get('website'),
                            'has_cba': stored_data.get('has_cba', False) or check_lender_cba(lender_name),
                            'fair_lending_violations': stored_data.get('fair_lending_violations'),
                            'redlining_complaints': stored_data.get('redlining_complaints')
                        }
    ...
    report_title = "Mortgage Lending in Tampa, Florida"
    ...
    community_profile = supporting_dir / "Community Profile of Tampa, FL (City, 2020).pdf"
    ...
    # Copy configuration
    config_file = member_reports_dir / "configs" / "tampa_market_report.json"
    ...
    data_dir = member_reports_dir / "data" / "processed"
    if (data_dir / "tampa_market_metrics.csv").exists():
        shutil.copy2(data_dir / "tampa_market_metrics.csv", report_folder / "tampa_market_metrics.csv")
        print(f"  Copied: tampa_market_metrics.csv")
    if (data_dir / "tampa_top_10_lenders_metrics.csv").exists():
        shutil.copy2(data_dir / "tampa_top_10_lenders_metrics.csv", report_folder / "tampa_top_10_lenders_metrics.csv")
        print(f"  Copied: tampa_top_10_lenders_metrics.csv")
    ...
    for file in ["tampa_market_data.csv", "tampa_top_lenders_2024.csv"]:
...
Loan Filters:
The analysis includes only conventional home purchase loans (loan_purpose = '1') for 
owner-occupied properties (occupancy_type = '1'), site-built construction (construction_method = '1'), 
1-4 family units, and excludes reverse mortgages. Only originated loans (action_taken = '1') are included.
