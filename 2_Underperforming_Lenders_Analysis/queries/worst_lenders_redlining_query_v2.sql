-- Worst Lenders Redlining Analysis Query v2
-- Identifies banks with worst redlining patterns for CBA enforcement
-- 
-- V2 Changes:
-- - Bank qualification: ≥500 apps in CBSA for 2022-2024 period only (accounts for census boundary changes)
-- - Output: Still includes all years 2018-2024 for historical comparison
-- - Peer matching: Independent by year and CBSA (matches peers separately for each year/CBSA combination)
--
-- Requirements:
-- - Top 200 CBSAs by total applications (2018-2024)
-- - Banks: $10B-$100B assets, ≥500 apps in CBSA (2022-2024 only for qualification), appropriate type_name
...
-- - Peer matching: 50%-200% of subject's volume by year/CBSA/loan_purpose/kind (independent for each year)
...
qualifying_banks_by_cbsa AS (
    -- Banks with ≥500 applications in CBSA across 2022-2024 period only
    -- This qualification threshold uses post-census boundary change years
    -- Note: All years 2018-2024 still included in final output for historical comparison
    SELECT
        b.lei,
        b.lender_name,
        c.cbsa_code,
        COUNT(*) as total_cbsa_applications_2022_2024
    FROM `hdma1-242116.hmda.hmda` h
    ...
    WHERE CAST(h.activity_year AS INT64) BETWEEN 2022 AND 2024  -- Qualification period: 2022-2024 only
      AND c.cbsa_code IS NOT NULL
      AND c.cbsa_code != '99999'
      -- Standard HMDA filters
    ...
    HAVING COUNT(*) >= 500  -- Minimum 500 applications in CBSA (2022-2024 total)
),

hmda_with_geography AS (
    -- Join HMDA data with tract demographics and filter to qualifying banks/CBSAs
    -- Includes all years 2018-2024 for historical comparison
    SELECT
        h.activity_year,
        h.lei,
        h.county_code,
    ...
    WHERE CAST(h.activity_year AS INT64) BETWEEN 2018 AND 2024  -- All years included in output
      AND c.cbsa_code IS NOT NULL
      AND c.cbsa_code != '99999'
      -- Standard HMDA filters
      AND h.occupancy_type = '1'
...
subject_volume AS (
    ...
    -- Peer matching happens independently for each year/CBSA combination
    SELECT
        activity_year,
        cbsa_code,
        county_code,
...
all_lenders_volume AS (
    ...
    -- Used to identify peers independently for each year/CBSA/county combination
    SELECT
        activity_year,
        cbsa_code,
        county_code,
...
peers AS (
    ...
    -- Peer matching is independent for each year, CBSA, county, loan_purpose_category, and kind
    -- This means the same bank may have different peers in different years or CBSAs
    SELECT DISTINCT
        al.activity_year,
        al.cbsa_code,
        al.county_code,
...
peer_data AS (
    ...
    -- Peer groups are specific to each year/CBSA/county/loan_purpose_category/kind combination
    SELECT
        e.activity_year,
        e.cbsa_code,
        e.cbsa_name,
