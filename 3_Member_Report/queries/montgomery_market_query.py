"""
Query builder for Montgomery County, MD Market Report
...
def build_montgomery_market_query(county_code: str, years: list) -> str:
    """
    Build market overview query for Montgomery County, MD
    
    Args:
        county_code: County FIPS code ('24031' for Montgomery County, MD)
    ...
    query = f"""
-- Montgomery County, MD Market Overview Query
...
WITH filtered_hmda AS (
    ...
    WHERE CAST(h.activity_year AS STRING) IN ({years_list})
        AND CAST(h.county_code AS STRING) = '{county_code}'
...
SELECT 
    ...
    mmct_loans
FROM lender_aggregated
WHERE lei IN (SELECT lei FROM top_10_lenders_2024)
